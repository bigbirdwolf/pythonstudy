package com.yltx.oil.partner.modules.oiltrade.presenter;

import com.yltx.oil.partner.modules.oiltrade.domain.GiftCardCardUseCase;
import com.yltx.oil.partner.mvp.controller.PageLimitPresenter;
import com.yltx.oil.partner.mvp.domain.UseCase;
import javax.inject.Inject;

/* loaded from: classes.dex */
public class GiftCardCardPresenter extends PageLimitPresenter {
    private GiftCardCardUseCase mGiftCardCardUseCase;

    @Override // com.yltx.oil.partner.mvp.controller.Presenter
    public void onPause() {
    }

    @Override // com.yltx.oil.partner.mvp.controller.Presenter
    public void onResume() {
    }

    @Inject
    public GiftCardCardPresenter(GiftCardCardUseCase giftCardCardUseCase) {
        this.mGiftCardCardUseCase = giftCardCardUseCase;
    }

    @Override // com.yltx.oil.partner.mvp.controller.PageLimitPresenter
    protected UseCase buildPageUseCase(int i, int i2) {
        this.mGiftCardCardUseCase.setPageOffset(i);
        return this.mGiftCardCardUseCase;
    }

    @Override // com.yltx.oil.partner.mvp.controller.Presenter
    public void onDestroy() {
        this.mGiftCardCardUseCase.unSubscribe();
    }
}